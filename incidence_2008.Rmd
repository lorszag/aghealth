---
title: "2008 USGS/SEER Comparison (All Sites)"
output:
...
incidence_data = read_excel('/Users/orszagl2/Library/CloudStorage/OneDrive-NationalInstitutesofHealth/2008_Incidence_Rates.xlsx')
```

```{r fix metolachlor & states, echo = FALSE, warning = FALSE}
df_2008$COMPOUND = gsub('.*METOLACHLOR.*', "METOLACHLOR", df_2008$COMPOUND)
...
incidence_total = incidence_total%>%inner_join(df_2008_grouped, by = c("STATE_FIPS_CODE", "COUNTY_FIPS_CODE"))
```
...
```{r}
incidence_total_all_can = incidence_rate%>%inner_join(df_2008_grouped, by = c("STATE_FIPS_CODE", "COUNTY_FIPS_CODE"))
